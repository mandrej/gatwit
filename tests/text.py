...
class HashTest(unittest.TestCase):
    ...
    def test_replace(self):
        ...
        if twit_link.search(self.text):
            self.text = twit_link.sub(r'<a class="twit" href="https://twitter.com/\1" target="_blank">@\1</a>', self.text)
        if hash_link.search(self.text):
            self.text = hash_link.sub(r'<a class="hash" href="https://twitter.com/search?q=%23\1&src=hash" target="_blank">#\1</a>', self.text)
        print self.text
